...
// Gráfica de radar/telaraña hexagonal comparando las 6 categorías de dos
// dispositivos. Hecha con SVG puro, sin librerías externas.
// ============================================================================
...
function polygonPoints(scores) {
  return CATS.map((c, i) => {
    const p = pointFor(i, scores[c.key]);
    return `${p.x},${p.y}`;
  }).join(" ");
}
  ...
  return (
    <div className="flex justify-center">
      <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} role="img" aria-label={`Gráfica comparando ${devA.name} y ${devB.name}`}>
        ...
        <polygon points={polygonPoints(devA.scores)} fill={colorA} fillOpacity={0.22} stroke={colorA} strokeWidth={2} />
        <polygon points={polygonPoints(devB.scores)} fill={colorB} fillOpacity={0.22} stroke={colorB} strokeWidth={2} />
